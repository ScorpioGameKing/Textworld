...
class HandbookParser:
    def parse_token_list(self, token_list):
        logger.debug(f"|PARSER| Token List: {token_list}")
        for _t in token_list:
            match _t[1]:
                case "SYS":
                    logger.debug(f"|PARSER| TOKEN: {_t, SystemCommands.COMMANDS.get(tuple(_t))}")
                    ...
                    match _t[0]:
                        ...
                        case "SPAWN_MOB":
                            if token_list.index(_t) + _func[1] + 1 <= len(token_list):
                                args = token_list[token_list.index(_t) + 1 : token_list.index(_t) + _func[1] + 1]
                                _func[0](gm=App.get_running_app().game.screens[1].game_manager, pos=[args[0][0], args[1][0]], name=args[2][0])
                                break
                            else:
                                logger.debug(f'Failed to spawn mob, invalid arg length')
                                break          
                case "ERROR":
                    logger.debug(f"|PARSER| Either this isn't set up or this isn't a command token: {_t}")
                    break